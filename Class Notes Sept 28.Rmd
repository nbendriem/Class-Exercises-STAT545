...
```{r}
ggplot(gapminder, aes(gdpPercap, lifeExp)) + geom_point() + geom_smooth(method="lm") + scale_x_log10()
```
...
```{r}
ggplot(gapminder, aes(gdpPercap, lifeExp)) + geom_point(alpha=0.1) + geom_point(y=predict(fit1), colour="red") + scale_x_log10()
```
...
```{r}
gapminder %>% group_by(country) %>% summarize(pop_growth=pop[year==1972]-pop[year==1962])
                                                    

gapminder %>% mutate(pop_bill = pop/1000000000)



gapminder %>% group_by(country) %>% mutate(lag_gdpPercap = lag(gdpPercap)) %>% filter(!is.na(lag_gdpPercap))
...
```{r}
gapminder %>% group_by(country) %>% mutate(max_gdpPercap=max(gdpPercap)) %>% ggplot(aes(year, pop/10^6)) + facet_wrap(~ continent) + geom_line(aes(group=country, colour=log(max_gdpPercap)), alpha=.25) + scale_x_continious("", breaks =c(1950, 1975, 2000)) + scale_colour_continous("log Maximum/nGPD per Cap.") + scale_y_log10("population(millions)"), breaks=c(0.1, 1, 10, 100, 1000),  + labs=c(0.1,1,10,100)) + labs(title="Population Trends") + theme(axis.text.x=element_text(angle=90), plot.title= element_text(hjust(0.5), legend.posotion= "bottom")
```
