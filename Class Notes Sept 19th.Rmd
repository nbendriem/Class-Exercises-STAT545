...
```{r}
gapminder %>% filter(continent=="Europe", gdpPercap > 10000) %>% select(-gdpPercap)
...
```
```{r}
mutate(gapminder, gdp= gdpPercap * pop, gpdBill= round(gdp/1000000000,1))
```
...
```{r}
mutate(gapminder,ngdp= gdpPercap * pop, 
...
```{r}
arrange(gapminder, country, desc(gdpPercap))
 ```
```{r}
gapminder %>% group_by(country) %>% summarize(gdpPercap)
...
```{r}
gapminder %>% group_by(country) %>% summarize(n_distinct(year))
```
...
```{r}
gapminder %>% filter(continent== "Asia") %>% group_by(year) %>% summarize(minexp=min(lifeExp), maxexp=max(lifeExp))
```
