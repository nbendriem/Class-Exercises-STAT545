...
```{r}
ggplot(gapminder, aes(x= year, y=lifeExp, colour=continent)) + geom_point() + geom_smooth(method="lm", se=FALSE)
```
...
```{r}
ggplot(gapminder, aes(x= year, y=lifeExp, colour=continent)) + geom_smooth(method="lm", se=FALSE)
```
...
```{r}
ggplot(gapminder, aes(gdpPercap, lifeExp)) +  geom_point(aes(colour=continent))
ggplot(gapminder, aes(gdpPercap, lifeExp)) +  geom_point(aes(shape=continent))
```
...
```{r, fig.width=8}
ggplot(gapminder, aes(gdpPercap, lifeExp)) + facet_wrap(~ continent) +  geom_point()
```
...
```{r}
ggplot(gapminder, aes(year, lifeExp)) + facet_wrap(~ continent) + geom_point() + geom_smooth(se=FALSE, span=0.5)
```
...
```{r, fig.width=8}
vc2 <- gapminder %>% mutate(size=c("small", "large")[(pop>7000000) + 1])  %>% ggplot(aes(gdpPercap, lifeExp)) + facet_grid(size ~ continent) 
vc2 + geom_point()
```
...
```{r, fig.width=8}
vc2 + geom_point(colour="brown", alpha=0.2) + geom_smooth() +  scale_x_log10()
```
...
```{r}
gapminder %>% filter(country=="Afghanistan") %>% arrange(year) %>% ggplot(aes(pop, gdpPercap)) + geom_point() + geom_path()
```
...
```{r}
ggplot(gapminder, aes(year, pop, group=country)) + geom_line(aes(colour=gdpPercap), alpha=0.33)
```


__Exercise 6__: Add points to the plot in Exercise 5.
```{r}
ggplot(gapminder, aes(year, pop, group=country)) + geom_line(aes(colour=gdpPercap), alpha=0.33) + geom_point()
```
